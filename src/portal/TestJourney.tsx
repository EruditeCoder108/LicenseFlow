...
export function TestPage({ applicationId, onStageChange }: { applicationId: string; onStageChange: StageChange }) {
  ...
  return <><Breadcrumbs applicationId={applicationId} current="LL knowledge-test simulation" /><div className="test-live-bar"><span><LockKeyhole size={16} />Answer checkpointing on</span><span><Signal size={16} />{media.snapshot.online ? 'Online' : 'Offline'}</span><span><Camera size={16} />{guided ? 'Guided signals' : mediaReady ? 'Camera ready' : 'Camera check required'}</span></div><section className="test-workspace"><div className="test-question-area"><div className="test-question-heading"><div><p className="eyebrow">Question {state.exam.currentQuestion + 1} of {demoQuestions.length}</p><h1 tabIndex={-1}>{question.prompt}</h1></div><span>{Object.keys(state.exam.answers).length} saved</span></div><fieldset className="test-answer-fieldset"><legend className="visually-hidden">Choose one answer</legend>{question.options.map((option, index) => <label className={selected === index ? 'selected' : ''} key={option}><input type="radio" name="test-answer" checked={selected === index} onChange={() => setSelected(index)} /><span>{String.fromCharCode(65 + index)}</span><strong>{option}</strong></label>)}</fieldset><div className="test-save-note"><LockKeyhole size={17} /><span>Your choice is not counted until you press “Save answer and next”.</span></div><button className="button button--primary" disabled={selected === null || !mediaReady} onClick={saveAnswer}>{state.exam.currentQuestion === demoQuestions.length - 1 ? 'Save answer and finish' : 'Save answer and next'} <ArrowRight size={18} /></button></div><aside className="test-observation-panel"><MiniCamera guided={guided} stream={media.stream} />{!guided && !media.snapshot.started && <button className="button button--secondary button--full" onClick={() => void media.start()}>Reconnect private checks</button>}<h2>Technical help only</h2><p>We can explain camera, saving and recovery. We cannot reveal or suggest an answer.</p><dl><div><dt>Payment</dt><dd>Recorded once</dd></div><div><dt>Latest checkpoint</dt><dd>Question {Math.max(1, Object.keys(state.exam.answers).length)} saved</dd></div></dl></aside></section></>
}
