...
async function reachPayment(page: Page) {
  ...
  await expect(page.getByRole('heading', { name: 'Check your device before you pay' })).toBeVisible()
...
test('prepared judge journey reaches a passing result and can reset cleanly', async ({ page, isMobile }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Before you start your test' })).toBeVisible()
  await page.getByRole('checkbox', { name: /I understand this is a demo test/ }).check()
  await page.getByRole('button', { name: 'Start judge walkthrough' }).click()
...
test('judge recovery lab exercises the protected server path', async ({ page, isMobile }) => {
  test.skip(isMobile, 'The full recovery proof runs once on desktop Chrome; mobile layout is checked separately.')
  test.setTimeout(90_000)
  await reachTestEntry(page)
  await page.getByRole('radio', { name: /Full test/ }).check()
  await page.getByRole('checkbox', { name: /I understand this is a demo test/ }).check()
  await page.getByRole('button', { name: 'Open full test' }).click()
  await page.getByRole('button', { name: 'Open the judge recovery lab' }).click()
  await expect(page).toHaveURL(/resilience=1/)
  await page.getByRole('checkbox', { name: /I understand this is a prototype test/ }).check()
  await page.getByRole('button', { name: 'Start test' }).click()
  await expect(page.getByText('Question 1 of 15', { exact: true })).toBeVisible()

  await page.getByRole('button', { name: 'Open recovery lab' }).click()
  const lab = page.getByRole('dialog', { name: 'Test recovery lab' })
  await lab.getByRole('button', { name: /Interrupt connection/ }).click()
  await expect(lab.getByText('The same test reconnected.')).toBeVisible()
  await expect(lab.getByText(/Same attempt · Question 1 · 0 confirmed answers/)).toBeVisible()
  await lab.getByRole('button', { name: /Try a second test client/ }).click()
  await expect(lab.getByText('The second client was blocked.')).toBeVisible()
  await expect(lab.getByText(/Same attempt · Question 1 · 0 confirmed answers/)).toBeVisible()
  await lab.getByRole('button', { name: 'Close recovery lab' }).click()

  await page.getByRole('radio').first().check()
  await page.getByRole('button', { name: 'Open recovery lab' }).click()
  await lab.getByRole('button', { name: /Lose save acknowledgement/ }).click()
  await expect(lab.getByText('The selected answer was saved once.')).toBeVisible()
  await expect(lab.getByText(/Same attempt · Question 2 · 1 confirmed answer/)).toBeVisible()
  await lab.getByRole('button', { name: /Reload during question/ }).click()
  const restoredLab = page.getByRole('dialog', { name: 'Test recovery lab' })
  await expect(restoredLab.getByText('The page reloaded without creating a new attempt.')).toBeVisible()
  await expect(restoredLab.getByText(/Same attempt · Question 2 · 1 confirmed answer/)).toBeVisible()
})
...
test('the synthetic interruption survives reload and resumes at question four', async ({ page, isMobile }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Your test is paused' })).toBeVisible()
  await page.reload()
  await expect(page.getByRole('heading', { name: 'Your test is paused' })).toBeVisible()
